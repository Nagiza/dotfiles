...
run_once("/home/nagiza/Downloads/copy/x86_64/CopyAgent")
...
-- {{{ Tags
tags = {
    names = { "1:web", "2:term", "3:editor", "4", "5:email" },
...
--{{{ A update function that will remove the program specific text from the tasklist (Does not work)
...
for s = 1, screen.count() do
    ...
    mytasklist[s] = awful.widget.tasklist(s, awful.widget.tasklist.filter.currenttags, mytasklist.buttons, myupdate)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({  }, "XF86AudioRaiseVolume",
        function ()
            os.execute("amixer -D pulse sset Master 5%+ unmute", volume.channel, volume.step)
            volume.update()
        end),
    ...
    awful.key({  }, "XF86AudioMute",
        function ()
            os.execute(string.format("amixer -D pulse sset Master toggle", volume.channel))
            volume.update()
        end),
